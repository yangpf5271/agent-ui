...
const InlineCode: FC<PreparedTextProps> = ({ children }) => {
  return (
    <code className="relative whitespace-pre-wrap rounded-sm bg-secondary/50 p-1">
      {children}
    </code>
  )
}

const CodeBlock = ({ children }: PreparedTextProps) => {
  const content = React.Children.toArray(children)
    .map((child) => {
      ...
      try {
        // Use Flatted to parse and stringify
        const parsed = parse(childContent)
  ...
  return (
    <pre className="relative my-2 whitespace-pre-wrap rounded-md bg-secondary/50 p-4 text-sm">
      ...
      <code className="mr-4 block break-words">{content}</code>
    </pre>
  )
}
